...
for ises = 1:Nsessions
    ...
    for a = 1:numel(visareas)
        ...
        RExtestnormscorecumpsthagg(ises).(whichvisarea) = (tempts-tempts(1,:))./(tempts(end,:)-tempts(1,:));
    end
...
save([pathsv 'SVMcumpsth' num2str(Twin) 'ms_', svmdesc '_' neuopt 'agg.mat'], 'SVMtrainRExcumpsthagg', ...
    'RExtestlabelfinalagg', 'RExtestscorecumpsthagg', 'RExtestnormscorecumpsthagg', 'RExtestTmilestonesagg', '-v7.3')
...
for itt = 1:numel(traintrialtypes)
    ...
    subplot(2,2,itt)
    hold all
    shadedErrorBar(cumpsthtl, mean(RExtestnormscorecumpsthagg(ises).(whichvisareaA)(:,trialsoind),2), ...
        std(RExtestnormscorecumpsthagg(ises).(whichvisareaA)(:,trialsoind),0,2)/sqrt(numel(trialsoind)), {'Color', 'b', 'LineWidth', 2},1)
    shadedErrorBar(cumpsthtl, mean(RExtestnormscorecumpsthagg(ises).(whichvisareaB)(:,trialsoind),2), ...
        std(RExtestnormscorecumpsthagg(ises).(whichvisareaB)(:,trialsoind),0,2)/sqrt(numel(trialsoind)), {'Color', 'r', 'LineWidth', 2},1)
